...
(async () => {
  ...
  for (const order of dataObjects) {
    console.log(`Processing order: ${order["Order no"]}`);

    try {
      // Check if page is still alive or if we need fresh context, recreate if needed
      if (
        !page ||
        !context ||
        page.isClosed() ||
        ...
        console.log(
          `Page/context closed, recreating for order: ${order["Order no"]}`
        );
        try {
          await context.close();
        } catch {}
      ...
      // Navigate to document manager and wait for it to load
      await page.goto("https://go.tradeshift.com/#/Tradeshift.DocumentManager");
      ...
      if ((await link.count()) === 0) {
        console.log(orderNo, ": This order not found");
        continue;
      }
      ...
      if (tdText.trim() === "RECEIVED") {
        ...
        console.log(`Successfully processed order: ${orderNo}`);

        // Wait to see the filled form before continuing
        console.log("Form filled - waiting 10 seconds to review...");
        await page.waitForTimeout(10000);

        // Force context recreation for next iteration
        try {
          await context.close();
        } catch {}
        context = null;
      ...
      continue; // Skip to next order if there's an error
    }
  }
